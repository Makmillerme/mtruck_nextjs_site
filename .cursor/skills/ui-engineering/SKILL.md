...
Примітиви — `user-shadcn` (`search_items_in_registries`). Готові секції/сторінки — `shadcnspace-mcp` (`searchBlocks` / `getBlockInstall`). Див. [`.cursor/rules/shadcnspace.mdc`](../../rules/shadcnspace.mdc).

Use `search_items_in_registries` MCP tool to find primitive components:
...
У цьому репо форми — **Zod + server actions / `useActionState`**. Не підключай React Hook Form, поки користувач явно не попросить.
...
Іконки: **react-icons**, не Lucide, якщо пакет не додано навмисно.
...
- [ ] Used existing action/useActionState pattern (no RHF unless asked)
...
❌ Adding React Hook Form/Lucide without an explicit request
